			...
			>
				...
				<div class="w-full h-full">
					{#if !toggleClick && outEnded}
						<span
							class="b-title w-full text-center font-extralight cursor-pointer text-shadow-effect"
							transition:fade
					...
					{#if toggleClick && outEnded}
						<div
							class="flex items-center justify-evenly gap-2 w-full h-full"
							transition:fade
							on:outroend={handleOutEnded}
							on:mouseleave={handleClick}
						>
							<a
								class="w-1/4 h-auto cursor-pointer"
								href="https://www.linkedin.com/in/gustavo-lee-228aa4223/"
								target="_blank"
								rel="noreferrer"
							>
								<svg
